...
以上为开发前快照。后续阶段①已实现，构建、运行与测试记录见 [阶段①验收记录](week02-stage1.md)；阶段②的执行器订阅也已实现，见 [阶段②验收记录](week02-stage2.md)；阶段③的服务和任务状态发布已实现，见 [阶段③验收记录](week02-stage3.md)；阶段④的一键启动已实现，见 [阶段④验收记录](week02-stage4.md)；阶段⑤的正常、错误、修复实验和实时图证据见 [阶段⑤验收记录](week02-stage5.md)；阶段⑥主项目验收与源码复现已完成，见 [阶段⑥记录](week02-stage6.md)；官方原版四组实验按用户要求暂缓。
